            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <Badge className="text-lg flex items-center gap-2">
                  ...
                  Score: {metrics?.overallScore ?? "7.5"}
                </Badge>
              </CardTitle>
              <p className="text-sm text-muted-foreground">
                Latest weekly supervisor assessment breakdown.
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-blue-600">9.0/10</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm flex items-center gap-2">
                      ⭐ Work Quality
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-green-600">9.5/10</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm flex items-center gap-2">
                      💬 Communication
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-blue-600">7.0/10</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm flex items-center gap-2">
                      ⚡ Efficiency
              ...
              <div className="pt-4 border-t">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-bold text-lg text-green-600">{metrics?.overallScore ?? "8.1"}/10</span>
                </div>
                <Progress value={(metrics?.overallScore ?? 8.1) * 10} className="h-2" />
                <p className="text-xs text-muted-foreground mt-1">
                  Assessment from week ending {new Date().toLocaleDateString()}
                </p>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">This week's daily time entries.</p>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, index) => {
                  const hours = index < 5 ? (7.5 + Math.random() * 2).toFixed(1) : index === 5 ? '4.0' : '0.0'
                  const isOvertime = parseFloat(hours) > 8
                  const isWeekend = index >= 5
                  const taskDesc = index < 5 ? ['Foundation prep', 'Concrete pouring', 'Rebar installation', 'Form removal', 'Site cleanup'][index] : index === 5 ? 'Equipment maintenance' : 'Rest day'
                  
                  return (
                    <div key={day} className={`flex justify-between items-center p-3 rounded-lg border ${isWeekend ? 'bg-gray-50' : 'bg-white'}`}>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <p className="font-medium text-sm">{day}</p>
                          {isOvertime && <Badge variant="outline" className="text-xs bg-orange-100 text-orange-800">OT</Badge>}
                        </div>
                        <p className="text-xs text-muted-foreground">{taskDesc}</p>
                        <p className="text-xs text-muted-foreground">
                          {hours !== '0.0' ? `7:00 AM - ${(7 + parseFloat(hours)).toFixed(0).padStart(2, '0')}:${((parseFloat(hours) % 1) * 60).toFixed(0).padStart(2, '0')} PM` : 'No work'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className={`font-semibold text-lg ${
                          hours === '0.0' ? 'text-gray-400' : isOvertime ? 'text-orange-600' : 'text-green-600'
                        }`}>
                          {hours}h
                        </p>
                        {parseFloat(hours) > 8 && (
                          <p className="text-xs text-orange-600">+{(parseFloat(hours) - 8).toFixed(1)}h OT</p>
                        )}
                      </div>
                    </div>
                  )
              ...
              <div className="mt-4 pt-4 border-t">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-lg font-bold text-green-600">42.5h</div>
                    <div className="text-xs text-gray-500">Regular Hours</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-orange-600">5.5h</div>
                    <div className="text-xs text-gray-500">Overtime</div>
                  </div>
                  <div>
                    <div className="text-lg font-bold text-blue-600">48h</div>
                    <div className="text-xs text-gray-500">Total Week</div>
                  </div>
                </div>
              </div>
            </CardContent>
